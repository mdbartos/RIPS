...
#     lonpos = pd.Series(np.arange(len(nc[yr].variables['longitude'][:])), index=nc[yr].variables['longitude'][:] - 360)
#     latlon = pd.Series(dem_ua['grid_cell'].dropna().unique()).str.split('_').str[1:].apply(pd.Series).astype(float)
#     latlonix = pd.concat([latpos[latlon[0].values].reset_index(), lonpos[latlon[1].values].reset_index()], axis=1)[0].dropna().astype(int)

#     dem_util = dem_ua.set_index('eia_code').sort_index().loc[idno]
#     util_d = {}

#     cat = np.ma.filled(nc[yr].variables['tasmax'][:,:,:], np.iinfo(np.int32).min)[:, latlonix.iloc[:, 0].values, latlonix.iloc[:, 1].values]
...
for i in reg_d.keys():
    proj_d.update({ i : {}})
    for k in nc.keys():
        proj_d[i].update({ k : project_reg(i, k)})

out_temps = pd.concat([pd.concat([i for i in proj_d[j].values()], axis=0).sort_index() for j in proj_d.keys()], axis=1)
out_temps.columns = [j for j in proj_d.keys()]

# Be careful with this
out_temps = out_temps[out_temps < 60].dropna(axis=1)

# Faster indexing?

latpos = pd.Series(np.arange(len(nc[2010].variables['latitude'][:])), index=nc[2010].variables['latitude'][:])
lonpos = pd.Series(np.arange(len(nc[2010].variables['longitude'][:])), index=nc[2010].variables['longitude'][:] - 360)
dem_ua = pd.read_csv('/home/akagi/github/RIPS_kircheis/data/util_demand_to_met_ua')
latlon = pd.Series(dem_ua['grid_cell'].dropna().unique()).str.split('_').str[1:].apply(pd.Series).astype(float)
latlonix = pd.concat([latpos[latlon[0].values].reset_index(), lonpos[latlon[1].values].reset_index()], axis=1)[0].dropna().astype(int)
...
def set_temps(slicelen):
    for i in latlonix.index.values[:slicelen]:
        ll = latlonix.iloc[i].values
        df[i] = nc[2010].variables['tasmax'][:, ll[0], ll[1]]



cat = np.ma.filled(nc[2010].variables['tasmax'][:,:,:], np.iinfo(np.int32).min)[:, latlonix.iloc[:, 0].values, latlonix.iloc[:, 1].values]
#cat = pd.Panel(cat, items=pd.date_range(datetime.date(2010,1,1), freq='d', periods=nc[2010].variables['tasmax'].shape[0]), major_axis=nc[2010].variables['latitude'][:], minor_axis=nc[2010].variables['longitude'][:] - 360)
